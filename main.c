...
/*
 * Main program.
 * Loads defaults, optional config, CLI overrides,
 * then bridges one KISS/TCP client to the LoRaHAM daemon socket.
 */
...
/*
 * Bridge loop:
 * One KISS/TCP client and one LoRaHAM daemon data socket.
 */
static int run_bridge(const lhkt_config_t *cfg, lhkt_stats_t *stats)
{
    int listen_fd;
    int client_fd;
    int data_fd;
